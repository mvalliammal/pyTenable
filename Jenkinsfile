...
try {
	...
	node('runPyPi') {
	  stage('runPyPi') {
	    try {
	      current_pytenable_version = sh (script: 'python -c "import tenable; print(tenable.__version__)" ',returnStdout:true).trim()
	      sudo apt-get install jq
	      pypi_pytenable_version = sh (script: 'curl -Ls https://pypi.org/pypi/$pythonPackage/json | jq -r .info.version', returnStdout:true).trim()
